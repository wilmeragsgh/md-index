...
class MDIndex:
    ...
    def generate(self):
        all_paths = list(
            filter(
                lambda x: all(map(lambda y: y[-3:] == ".md", x[2])) and len(x[2]) > 0,
        ...
        tst_out = "\n".join(
            list(map(lambda x: self._parse_toc(0, x), list(self.paths_dict)))
        )
    ...
    def _parse_toc(self, ident, key) -> str:
        ...
        if len(current_val["files"]) > 0:
            files = "\n".join(
                [
                    f'{(ident+1)*4*" "}* [{fil.replace(".md","").capitalize().replace("-"," ").replace("_"," ")}]({key[2:]}/{fil})'
                    for fil in current_val["files"]
                    if fil != "README.md"
                ]
            )
            within_dirs = (
                ...
                + [files]
                + [
                    self._parse_toc(ident + 1, f"{key}/{new_key}")
                    for new_key in current_val["dirs"]
                ]
            )
        else:
            within_dirs = [root] + [
